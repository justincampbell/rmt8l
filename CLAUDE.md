...
internal/proto/proto.go       — sentinel/chunk walker, parameter parser
internal/proto/render.go      — diff-friendly txt renderer (byte-equivalent to the Python prototype)
internal/proto/testdata/      — golden fixture: tx-settings.{bin,txt} from a real T8L
```

`internal/proto/proto_test.go` decodes the fixture and asserts the rendered txt matches byte-for-byte. This is load-bearing — once it passes, live pulls are mechanical.

Subcommands (current): `backup`, `ports`, `version`.

Reserved subcommand names (don't repurpose without renaming): `info` (richer device-state packet — issue #1), `channels` (live stick/switch snapshot — issue #3), `set`, `restore` (write path — issue #5).

`backup` sends `A5 11 00 0D 0A` and writes two files into `--out-dir` (default `.`): `tx-settings.bin` (raw response, byte-stable) and `tx-settings.txt` (decoded view, one record per block, settings sorted by id so `git diff` localises changes). `--from-file` decodes an existing .bin without touching the radio — useful for tests and for re-rendering after a renderer change.
...
- **Factory reset semantics are unclear.** A "factory reset" via the web UI did not change anything visible in our `A5 11` dump — not even the bind UID `d84313`, which we'd expect to re-randomize. Either reset only touches radio-side state (which lives in the `0xFF` device-state packet, issue #1), or the reset was no-op, or there's a field we're missing. Worth re-checking once #1 lands.
...
| `0x55` | "general" subsystem: channels, volume, calibration, model-match, etc. Many commands; only a few decoded (issue #5). |
...
- `Radiomaster-RC/RM-Web-Page` on GitHub — `Connection.html` is the authoritative protocol source. Key functions: `parseParamData` (param type table), `parseDeviceInfoPacket` (`0xFF` device state — issue #1), `tryParseChannelMapPacket` (`0x44` — issue #2), `readLoop` (top-level dispatcher showing all packet prefixes).
- `~/Code/justincampbell/fpv/CLAUDE.md` — has a section on the T8L documenting the M+power gotcha and the dump pipeline; also has channel-map war stories from the Radiomaster Pocket worth reading before tackling issue #2.
